...
func TestAccountSwitch(t *testing.T) {
	...
	t.Run("adopt copies auth to account dir", func(t *testing.T) {
		townRoot, accountsDir := setupTestTownForAccount(t)
		...
		// Create ~/.claude as a real directory with auth files
		claudeDir := filepath.Join(fakeHome, ".claude")
		if err := os.MkdirAll(claudeDir, 0755); err != nil {
			t.Fatalf("mkdir .claude: %v", err)
		}
		// Write .claude.json (auth essential)
		if err := os.WriteFile(filepath.Join(claudeDir, ".claude.json"), []byte(`{"auth":"token"}`), 0600); err != nil {
		...
		// Write a non-auth file that should NOT be copied
		if err := os.WriteFile(filepath.Join(claudeDir, "history.jsonl"), []byte("log entry"), 0644); err != nil {
		...
		// Run adopt
		cmd := &cobra.Command{}
		err := runAccountAdopt(cmd, []string{"work"})
		if err != nil {
			t.Fatalf("runAccountAdopt failed: %v", err)
		}

		// Verify auth files were copied
		if _, err := os.Stat(filepath.Join(workConfigDir, ".claude.json")); err != nil {
			t.Error(".claude.json not copied to account dir")
		}
		if _, err := os.Stat(filepath.Join(workConfigDir, "settings.json")); err != nil {
			t.Error("settings.json not copied to account dir")
		}

		// Verify non-auth file was NOT copied
		if _, err := os.Stat(filepath.Join(workConfigDir, "history.jsonl")); err == nil {
			t.Error("history.jsonl should not have been copied")
		}

		// Verify original ~/.claude is untouched
		if _, err := os.Stat(filepath.Join(claudeDir, ".claude.json")); err != nil {
			t.Error("~/.claude/.claude.json should still exist (copy, not move)")
		}
	})
