...
func Scan(db *sql.DB, dbName string, maxAge, purgeAge, mailDeleteAge, staleIssueAge time.Duration) (*ScanResult, error) {
	...
	// A dangling parent ref means the wisp_dependencies row has a typed target
	// (depends_on_issue_id / depends_on_wisp_id / depends_on_external) that no
	// longer resolves. External targets have no FK to check; we only flag rows
	// where the *populated* internal target column doesn't resolve.
	danglingQuery := `
		...
		WHERE wd.type = 'parent-child'
		AND (
			(wd.depends_on_wisp_id IS NOT NULL AND pw.id IS NULL)
...
func batchDeleteRows(ctx context.Context, db *sql.DB, idQuery string, cutoffArg time.Time, primaryTable string, auxTables []string) (int, error) {
	...
	for {
		...
		// Targets are typed (depends_on_issue_id / depends_on_wisp_id / depends_on_external);
		// the deleted IDs could appear in any of the three target columns. Bind args 3x.
		delReverse := fmt.Sprintf("DELETE FROM wisp_dependencies WHERE depends_on_issue_id IN %s OR depends_on_wisp_id IN %s OR depends_on_external IN %s", inClause, inClause, inClause)
		revArgs := make([]interface{}, 0, len(args)*3)
		revArgs = append(revArgs, args...)
		revArgs = append(revArgs, args...)
		revArgs = append(revArgs, args...)
		if _, err := db.ExecContext(ctx, delReverse, revArgs...); err != nil {
			// Non-fatal.
		}
